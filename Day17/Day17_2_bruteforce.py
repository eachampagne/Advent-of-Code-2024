...
#Solved Dec. 17, 2024
#This brute force solution has been deprecated! It works on the example, but probably won't finish the actual problem in my lifetime
#I tried just testing every single solution in order
#It was worth a try before I broke out the pen and paper
#Once I estimated that the solution would be in the 8^16 range I gave up
#Keeping this file for posterity, I guess
...
while not solution_found:
    if test_soln % 1000 == 0:
        print(test_soln)
    computer = Computer3Bit(test_soln, regB, regC, program, False)
    results = computer.run()
    if results == program_string:
        solution_found = True
    else:
        test_soln += 1
