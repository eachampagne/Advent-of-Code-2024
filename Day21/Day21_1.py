...
def find_number_presses(starting_key, ending_key, iteration):
    if iteration == 3:
        return 1
    if iteration == 0:
    ...
    for i in paths:
        ...
        for j in i:
            next_key = j
            presses += find_number_presses(current_key, next_key, iteration + 1)
            current_key = next_key
        presses_per_path.append(presses)
    return min(presses_per_path)
...
for i in f:
    ...
    for j in code:
        ...
        length += find_number_presses(starting_key, ending_key, 0)
