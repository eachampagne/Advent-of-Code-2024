        ...
        if not_ordered: #fix out of order lists
            ...
            while len(pages) != 0: #move pages from "to sort" pile (pages) to "sorted" pile (ordered_pages)
                ...
                if not is_sorted: #it is possible (at least in theory, though maybe not in my input data) that there isn't yet enough information for there to be a unique place to sort a page. In this case, place it at the end of the "to sort" pile and return later
                    #I coded my algorithm wrong and it actually inserts each page at the first slot that doesn't break any rules, even if that's not the only slot, so this code never runs. It happens that with my input file I get the right final answer anyway (I guess I might have stuff in the wrong order, but as long as the middle term is right it doesn't matter). However I can think of edge cases that my code would not work for - I'm working on a more complete algorithm in a different script.
